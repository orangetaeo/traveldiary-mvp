...
export default function WrapUpError({ reset }: ErrorPageProps) {
  return (
    <div className="min-h-screen bg-surface-soft flex flex-col items-center justify-center px-td-md text-center">
      <span className="material-symbols-outlined text-6xl text-ink-mute mb-td-md">celebration</span>
      <h1 className="text-xl font-bold text-ink mb-td-xs">마무리 페이지 로딩 실패</h1>
      <p className="text-td-body text-ink-soft max-w-xs leading-relaxed">
        여행 마무리 페이지를 불러오지 못했어요.
      </p>
      <button
        type="button"
        onClick={reset}
