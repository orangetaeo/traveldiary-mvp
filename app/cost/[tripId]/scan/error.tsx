...
export default function ScanError({ reset }: ErrorPageProps) {
  return (
    <div className="min-h-screen bg-surface-soft flex flex-col items-center justify-center px-td-md text-center">
      <span className="material-symbols-outlined text-6xl text-ink-mute mb-td-md">receipt_long</span>
      <h1 className="text-xl font-bold text-ink mb-td-xs">스캔 오류</h1>
      <p className="text-td-body text-ink-soft max-w-xs leading-relaxed">
        영수증 스캔 화면을 불러오지 못했어요.
      </p>
      <button
        type="button"
        onClick={reset}
