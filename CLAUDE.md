...
### Magic Moments (v2 — 2026-05-03 사이클 ZZ 갱신, 8개)

| # | 모먼트 | 화면 | 담당 에이전트 | 핵심 스킬 | 상태 |
|---|--------|------|--------------|----------|------|
| M1 | 추천 근거 패널 | 일정 상세 | T1, T3, T4 | S-02, S-03, S-05 | ✅ 사이클 1 + 5b-3/5b-6 (Google/Naver 실 검증) |
| M2 | D-Day 모드 전환 | 여행 중 홈 | T7, T17 | S-04 | ✅ 사이클 3 → 5b-4 (Geolocation) → WW (베트남 6 도시 boundary) |
| M3 | Live Replan | 재계획 모달 | T2, T5 | S-01, S-06 | ✅ 사이클 2 → 5b-2 (commitReplan mutation 영속화) |
| M4 | 카메라 번역 | 번역 화면 | T6 | S-07, S-08 | ✅ 사이클 4 → 5b-5 (Vision OCR + Claude API 실 동작) |
| M5 | 응급/실용/도시 컨텍스트 | /travel 푸터 + /city | T8, T16 | S-20 | ✅ 사이클 8 + P (응급 강화 — /city/[slug]/emergency 분실 가이드 + CityContextStrip) |
| M6 | D-Day 체크리스트 + 비용 관리 | /checklist /cost | T8, T17 | S-21 | ✅ 사이클 9 + UU (정산 마커 ADR-042) + XX (시드 trip 자동 부트스트래핑 ADR-044) |
| M7 | 공유 링크 + 동기화 키 협업 | /share /shared | T9, T16 | S-22 | ✅ 사이클 11a~c + R (ShareComment 댓글/리액션) + SS (내 정보) + YY (본인 활동) |
| M8 | OTA 가격 비교 (수익 모델) | 일정 상세 인라인 | T9, T10 | S-23 | ✅ 사이클 12a~c + N + VV (베트남 한정 활성, 비-VN OTA seed dormant) |

> v2 비전 — [docs/09-vision-v2.md](docs/09-vision-v2.md) (Accepted 2026-04-29)
> 베트남 단일 국가 정책 — auto-memory `feedback_vietnam_only_focus` (2026-05-03 사이클 ZZ)
